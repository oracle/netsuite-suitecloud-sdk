...
import { SUITECLOUD_PROXY } from '../../service/TranslationKeys';
...
export default class PanelPresenter {
	...
	setRunningStatus(): void {
		this._statusBarItem.text = `$(netsuite-mobius-colorless-icon)  ${this._translationService.getMessage(SUITECLOUD_PROXY.IS_RUNNING.STATUSBAR)}`;
		this._showStatusBarItem();
	}

	setStoppedStatus(): void {
		this._statusBarItem.text = `$(netsuite-mobius-colorless-icon)  ${this._translationService.getMessage(SUITECLOUD_PROXY.IS_STOPPED.STATUSBAR)}`;
		this._showStatusBarItem();
	}
