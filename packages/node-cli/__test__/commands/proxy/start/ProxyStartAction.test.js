...
describe('ProxyStartAction execute()', () => {
	...
	it('should show stop instructions after proxy starts successfully', async () => {
		...
		expect(log.info).toHaveBeenNthCalledWith(6, '  * Base URL: http://127.0.0.1:8383/api/internal/devassist');
		expect(log.info).toHaveBeenNthCalledWith(7, '  * API Key: Use the key generated by "suitecloud proxy:generatekey".');
		expect(log.info).toHaveBeenNthCalledWith(8, '  * Model ID: netsuite');
		expect(log.info).toHaveBeenNthCalledWith(9, '');
		expect(log.info).toHaveBeenNthCalledWith(10, 'Press Ctrl+C to stop the proxy.');
	});
});
