...
describe('ProxyStartAction execute()', () => {
	...
	it('should show stop instructions after proxy starts successfully', async () => {
		...
		expect(log.result).toHaveBeenCalledWith('SuiteCloud Developer Assistant proxy is running on port 8383 with auth ID "defaultAuth".');
		expect(log.info).toHaveBeenNthCalledWith(2, '');
		expect(log.info).toHaveBeenNthCalledWith(3, 'To use it on this machine, configure your Coding Agent as follows:');
		expect(log.info).toHaveBeenNthCalledWith(4, '  * API Provider: OpenAI Compatible');
		expect(log.info).toHaveBeenNthCalledWith(5, '  * Base URL: http://127.0.0.1:8383/api/internal/devassist');
		expect(log.info).toHaveBeenNthCalledWith(6, '  * API Key: Use the key generated by "suitecloud proxy:generatekey".');
		expect(log.info).toHaveBeenNthCalledWith(7, '  * Model ID: netsuite');
		expect(log.info).toHaveBeenNthCalledWith(8, '');
		expect(log.info).toHaveBeenNthCalledWith(9, 'Press Ctrl+C to stop the proxy.');
	});
});
