...
describe('validateFolderAlreadyExists', () => {
	it('should return true when folder does not exists', () => {
		expect(validateFolderAlreadyExists('/InteractiveAnswersValidator-validateFolderAlreadyExists')).toEqual(positiveResponse);
	});
	...
	it('should return false when folder already exists', () => {
		...
		const tmpDir = path.join(os.tmpdir(), '/InteractiveAnswersValidator-validateFolderAlreadyExists');
		try {
			fs.mkdirSync(tmpDir);
			expect(validateFolderAlreadyExists(tmpDir)).toEqual('The folder ' + tmpDir + ' already exists.');
		} finally {
			fs.rmdirSync(tmpDir);
		}
	});
