...
module.exports = class ImportConfigurationOutputHandler extends BaseOutputHandler {
	parse(actionResult) {
		...
		if (successfulImports.length) {
			this._log.result(NodeTranslationService.getMessage(COMMAND_CONFIGIMPORT.OUTPUT.IMPORTED));
			successfulImports.forEach((item) => this._log.result(`${item.type}:${item.id}`));
		} else {
			this._log.result(NodeTranslationService.getMessage(COMMAND_CONFIGIMPORT.OUTPUT.NONE_IMPORTED));
		}
		if (failedImports.length) {
			this._log.error(NodeTranslationService.getMessage(COMMAND_CONFIGIMPORT.OUTPUT.NOT_IMPORTED));
