...
function validateExecutionInput(input: ProjectCommandExecutionInput): void {
	...
	if (!Object.values(PROJECT_COMMAND).includes(input.command)) {
		throw new Error(translationService.getMessage(TranslationKeys.PROJECT_COMMAND.ERROR.UNSUPPORTED_COMMAND, input.command));
	}
	if (!input.projectFolder) {
		throw new Error(translationService.getMessage(TranslationKeys.PROJECT_COMMAND.ERROR.PROJECT_FOLDER_REQUIRED));
	}
	if (!input.hostName) {
