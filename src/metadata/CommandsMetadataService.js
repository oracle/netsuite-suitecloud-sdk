...
class CommandsMetadataService {
	...
	_addCommandGeneratorMetadata(commandGeneratorsMetadata, commandsMetadata) {
		executeForEachCommandMetadata(commandsMetadata, commandMetadata => {
			...
			commandMetadata.interactiveGenerator =
				generatorMetadata && generatorMetadata.interactiveGenerator
	...
	getCommandsMetadata() {
		var sdkCommandsMetadata = this._getMetadataFromFile(
			ApplicationConstants.SDK_COMMANDS_METADATA_FILE
		);
		var nodeCommandsMetadata = this._getMetadataFromFile(
			ApplicationConstants.NODE_COMMANDS_METADATA_FILE
		);
		var commandGeneratorsMetadata = this._getMetadataFromFile(
			ApplicationConstants.COMMAND_GENERATORS_METADATA_FILE
		);
